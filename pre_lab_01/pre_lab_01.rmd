...
In the space below, write the data type for the "id" column and the "seats" column (it's in light gray, offset by \< \>). **Answer**"id" column is chr and "seats" is dbl
...
In the space below, write the name of the business from the first row of the "name" column. **Answer**What business and I don't see a name column? Maybe I missed something, first row of the "id" column is LING889. 
...
**Answer**The first line calls the dataset, the second declares one field as "term," and the next two give the sum of another field, "classes," grouped in terms of the first field.
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer**The arrange function was added. It ordered the sums of classes per term so that they're descending instead of in the first code where it's ordered by term chronologically. 
...
**Answer**The "department" field was added to the group_by function, as well as the arrange funciton along with term without any desc modifier. The name of the academic departments was added, and the order seems to be the alphabetical order of departments. 
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer**The total seats field was calculated and added to the summarise function, and added with the desc modifier to the arrange function. Now the summary is ordered in descending total seats and includes the total classes corresponding to the same department during the same term. 
...
Compared to the previous codeblock, what has changed and how did that affect the result? What do the top terms have in common? **Answer**Means and medians of seats were added to the summary. The top four terms are fall. 7 of the top ten are fall terms. 
...
**Answer**The data is sorted the same way, by total seats descending. But it's grouped by academic department instead of term, and fields for the minimum and maximum seats for those departments were added.
...
1.  What instructors were more popular? i.e. which had more proportionally filled seats.

2.  What department is most popular, and when?

3.  What is the proportion of seats:classes for the different departments? Which has the smallest classes and which has the largest?
