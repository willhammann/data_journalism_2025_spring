...
```{r}
# read in the html
the_banner <- "https://www.thebanner.com/baltimore/"
...
divs <- results |>
  rvest::html_elements(xpath = "//*div[contains(@class, 'headline')]") |>
  rvest::html_text() |>
  clean_names() |>
  set_names("headlines")

h3s <- results |>
  rvest::html_elements(xpath = "//*h3[contains(@class, 'headline')]") |>
  rvest::html_text() |>
  clean_names() |>
  set_names("headlines")

h2s <- results |>
  rvest::html_elements(xpath = "//*h2[contains(@class, 'headline')]") |>
  rvest::html_text() |>
  clean_names() |>
  set_names("headlines")

text <- rbind(divs, h2s, h3s)
...
releases <- osp_resuls |>
  ...
  separate(data, c("date", "title_two"), sep = "-") |>
  #same with em dash
  mutate(title = case_when(
    !is.na(title) ~ title,
    !is.na(title_two) ~ title,
    TRUE ~ title_three
  )) |>
  select(-title_two)
  ))
