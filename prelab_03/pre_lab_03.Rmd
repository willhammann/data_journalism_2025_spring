...
**Task** Load some [Maryland state government payments data](https://opendata.maryland.gov/Budget/State-of-Maryland-Payments-Data-FY2008-to-FY2024/7syw-q4cy) by running the following code. We'll use the guess_max() function as an argument to use the first 10 rows to set the data type. What does the first line of the red Warning message that prints out when you load the data say? What happens when you follow its instructions? Answer below. **Answer**"One or more parsing issues, call 'problems()' on your dataframe for details," and "ℹ Use `spec()` to retrieve the full column specification for this data."
I did problems(payments) first and was confused but spec(payments) shows the datatype for each column in the dataset. Character data is red and double is green, which I think is just to diferentiate them. 

```{r}
payments <- read_csv("C:/Users/willi/OneDrive/Documents/GitHub/data_journalism_2025_spring/prelab_03/data/State_of_Maryland_Payments_Data__FY2008_to_FY2024_20250115.csv", guess_max=10)
```

### Task 3: Reload data

**Task** Run the following codeblock to reload the data, using every row to set the data types. Does it show any parsing errors when you run? Answer below **Answer**Nope.
...
**Task** Run the following codeblock to glimpse the data. What data type is the "Vendor Name" field? What data type is the "Amount" field? What data type is the "Date" column? Answer below. **Answer**Chr data type. 
...
**Task** Run the following codeblock to group by the vendor's zip code and show any that are less than five characters in length. How many zip codes do not have five characters? What do you think explains those records? **Answer**553 zip codes are less than five characters. Maybe these are foreign systems that international vendors use? The bottom of the dataset shows values like "V6V" as a zip code, so maybe that's a different system entirely. 
...
```{r}
md_grants_loans <- read_csv("C:/Users/willi/OneDrive/Documents/GitHub/data_journalism_2025_spring/prelab_03/data/State_of_Maryland_Grant_and_Loan_Data__FY2009_to_FY2022_20250115.csv")
```
...
**Task** Run the following codeblock and describe below what each line is doing. Looking at the results, what jumps out at you as potentially problematic? **Answer**There is a small number of NA values in the category field, as well as a mysterious one called "L" which may just mean Loan, and another called "contract" which could probably be considered a grant but maybe not. 
...
**Task** Run the following code to arrange the grants & loan data so that the smallest amount is first. Describe what you see. **Answer**I don't notice anything with the data arranged in this order as opposed to how it was arranged before. I've just noticed that Fiscal Period is always 1 and Date is always on June 30 of whatever the year is, so maybe those are oddities. 
...
**Task** Run the following codeblock to use the `clean_names()` function from janitor to standardize column names. How does it change the headers? Answer below. **Answer**It makes everything lowercase and replaces spaces with underscores. 
...
**Task** Run the following codeblock to use str_to_upper() to make the source, grantee and description columns upper-cased. Describe what each line is doing. **Answer**A new dataframe is created. clean_names() is called to standardize the names, then the grantor field is renamed to source, and then the values of fields source, grantee and description are made uppercase. 
...
**Task** Run the following codeblock to check for duplicate rows using get_dupes(). How many duplicates are possible? **Answer**There are 58 dupes of several rows. 
...
**Task** Run the following codeblock to use distinct() to get rid of duplicate rows. How many rows does the new dataframe have? Answer below. **Answer**19,453 rows. 
...
**Task** Run the following codeblock to use str_sub() to convert the ZIP codes that have nine digits to five digits, creating a new column. Look at the difference in the result - what changed? **Answer**All the zip codes are five digits in that column. 
