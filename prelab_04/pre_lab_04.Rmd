...
1.  Ensure students install refine package and load it.
...
**Task** Run the following code to group and count records in the Grantee column, and filter to examine only recipients that start with W. Name a problem that you see that is preventing proper grouping. **Answer**Rows have duplicates such as Washington County Commission on Aging due to caps or no caps, etc.
...
**Task** Run the following code to make a copy of the Grantee field called grantee_clean, then use the `key_collision_merge` function from refinr to attempt to standardize similar values. Store it as a new object called cleaned_md_grant_loans Click the black arrow on the table to examine the Grantee and grantee_clean columns. Looking at the results on the first 3 pages only (first 30 rows), name two changes the refinr package made, by comparing Grantee to grantee_clean. **Answer**Some rows but not all have been visibly standardized into all caps versions, likely where that was one of several dupes.
...
**Task** Run the following code to group by grantee_clean and Grantee, then filter for recipients that start with W. Name three changes the refinr function made in the first 30 results. **Answer**Washington County Commission on Aging, Washington College, and even Walden Sierra Inc. have all had duplicates consolidated. The cleaned column has nine less rows. 
...
**Task** Run the following code to use the n_gram_merge() function to clean the data, then group by grantee_clean and Grantee to examine recipients that start with W. Look at the differences. Does the changes it made vary substantially from the previous use of key_collision_merge? **Answer**It looks like this has three rows less, so the function was able to just slightly cut down on things for a second time. 
