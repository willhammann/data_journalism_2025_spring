...
**Task** Run the following code to load data. Describe what you see in the answer space below. What columns exist in this data? **Answer**The data has columns for ID or FIPS, five character zipcode, municipality name, and fields setting the features' geometry as polygons and defining the polygons' size with length and area. The fields I'm confused on are the 10-11 character long zipcodes and the EXISTING column.
...
**Task** Run the following code. Describe the output in the space below: what kind of information does it contain? **Answer**What I said in the last question. Each row is a polygon representing a zipcode that will appear in the layer were you to upload the shapefile into ArcGIS or something. 
...
**Task** Run the following code. Describe the output in the space below. **Answer**Looks like every zip code in Maryland as the layer makes up the state's borders imposed on graticule. 
...
**Task** Run the following code. Describe the output in the space below. **Answer**It asks me to enable caching of data using a line of code it gives me, so I added it below and ran it.  
...
**Task** Run the following code. Describe the output in the space below. **Answer**Another shapefile dataset with polygons representing U.S. counties. 
...
**Task** Run the following code. Describe the output in the space below. **Answer**It pulled the Maryland counties from the above dataset. 
...
**Task** Run the following code. Describe the output in the space below. **Answer**Just like the map of Maryland zip codes except the borders show our counties instead. Also some polygons include parts of the Chesapeake. 
...
**Task** Run the following code. Describe the output in the space below. **Answer**Both layers are shown in this map, although they have the same symbology so it's a bit ambiguous. 
...
**Task** Run the following code. Describe the output in the space below. **Answer**A dataset with only two columns, one for zip code and one for amount of foreclosure notices in March 2023 for that zip code. 
...
**Task** Run the following code. Describe the output in the space below. **Answer**The shapefile of zip codes now includes a field for the amoutn of notices to forclose in March 2023. 
...
**Task** Run the following code. Describe the output in the space below. **Answer**The plot returns the zip code shapefile as a choropleth that show where more foreclosure notices are appearing. 
...
**Task** Run the following code. Describe the output in the space below. Be sure to input your census api key. **Answer**My libraries loaded. I didn't input my api key because it always tells me it's already saved. 
...
**Task** Run the following code. Describe the output in the space below. **Answer**Two dataframes, one with MD counties and their populations in 2021, and the other with total notices to forclose per county in MD.
...
**Task** Run the following code. Describe the output in the space below. **Answer**The county population dataframe was updated with geometry data, classifying rows as polygons. 
...
**Task** Run the following code. Describe the output in the space below. **Answer**You can see the above discrepancy in county names in the dataframes. 
...
**Task** Run the following code. Describe the output in the space below. **Answer**The county column was renamed as such and the rows made into title case without the name of the state. Also the "estimate" column was renamed to "population."
...
**Task** Run the following code. Describe the output in the space below. **Answer**Population and foreclosure total columns are included with the county shapefiles. 
...
**Task** Run the following code. Describe the output in the space below. **Answer**Made a new field for the rate of notices per 1000 people. 
...
**Task** Run the following code. Describe the output in the space below. **Answer**The choropleth shows the distribution of foreclosure rates in different counties, with messy labels. 
...
**Task** Run the following code. Describe the output in the space below. **Answer**The same map was made with a clearer, diverging color scheme, and this time no county labels. 
...
**Task** Run the following code. Describe the output in the space below. What regional patterns do you see? **Answer**This color scheme has one less class than the last one. Highest rates are concentrated in Southern Maryland. 
