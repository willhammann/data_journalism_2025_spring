...
**A1.**Friends of Jain Ashwani spent the most at $62357.38
...
refine_media_expenses <- read_csv("data/refine_media_expenses.csv")

refine_media_expenses |>
  filter(str_detect(payee_clean, "Facebook")) |>
  group_by(committee_name) |>
  summarise(total_fbspending = sum(amount)) |>
  arrange(desc(total_fbspending))
...
**A2.**Candidates ran for Congress, Governor, Harford County Exec, Baltimore City Board of School Commissioners, PG County Council. PAC's advocate for immigrants and people of color and working class folk. Ads for top spender Jain Ashwani are various general political ads relating to his run for a representative of Congress for Maryland in 2024.  
...
**A3.**Wes Moore and Aruna Miller are notably not on that list. They didn't show up on the A1 dataset so they probably didn't buy facebook ads. The biggest recipient of their media expenses is SKDK, a political consulting firm which probably ran his campaign and maybe paid other sites for advertisements indirectly with the funds provided to them.  
...
**A4.**The top total spending for a committee's online advertising is 930681.11, versus 62357.38 for facebook, so groups spent much more on a varied approach. But that top number for Facebook spending came from Jain Ashwani, whose total spending equaled that number. Peter Franchot spent a majority of his ad spending on Facebook, but not all of it like Ashwani. 

```{r}
refine_media_expenses |>
  filter(str_detect(expense_purpose, "Online Advertising")) |>
  group_by(committee_name) |>
  summarise(total_advert = sum(amount)) |>
  arrange(desc(total_advert))
```
...
**A5**We know that two candidates for governor spent all or the majority of their online ad spending on Facebook ads. Jain got 2% and Franchot got 21% of the vote. We don't know how much Wes Moore spent on Facebook because I believe that SKDK would have spent on various places but we don't see that here. I'm not sure if there's anything that significant but I think if anything I'd do a story on Ashwani's campaign since they uniquely went all in on Facebook, and do a deeper dive on that strategy and how it went. 
