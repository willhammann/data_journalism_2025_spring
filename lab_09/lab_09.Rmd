...
md_gov_cox_hogan <- md_gov_county_22 |>
  ...
  clean_names() |>
  mutate(
    total_votes_22 = cox + moore + lashar + wallace + harding + sefcik + write_ins_x,
    percent_cox = cox / total_votes_22 * 100,
    total_votes_18 = hogan + jealous + quinn + schlakman + write_ins_y,
    percent_hogan = hogan / total_votes_18 * 100
  )

md_gov_cox_hogan <- md_gov_cox_hogan |>
  select(-county_y) |>
  rename(county = county_x, write_ins_cox = write_ins_x, write_ins_hogan = write_ins_y)
